...
const DashboardLayout = ({
  ...
  return (
    <SidebarProvider>
      <div className={`min-h-screen flex w-full bg-gradient-to-br ${colorScheme === "dark" ? 'from-slate-900 via-slate-800 to-slate-900' : 'from-slate-50 via-blue-50 to-slate-100'}`}>
        <AppSidebar activeTab={activeTab} onTabChange={onTabChange} />
        <SidebarInset className="flex-1">
          <nav className="border-b border-border/50 bg-card/80 backdrop-blur-sm sticky top-0 z-10">
            <div className="flex justify-between items-center h-16 px-4 lg:px-6">
              <div className="flex items-center space-x-4">
                <SidebarTrigger className="lg:hidden" />
                <div className="hidden sm:block">
                  <h1 className={`text-xl lg:text-2xl font-bold bg-clip-text text-transparent ${colorScheme === "dark" ? 'text-dark' : 'text-light'}`}>
                    StenoLearn
                  </h1>
                </div>
              </div>
