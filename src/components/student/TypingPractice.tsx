...
const TypingPractice = ({ assignments }: TypingPracticeProps) => {
  ...
  const handleSubmit = () => {
    ...
    const accuracy = calculateAccuracy(typedText, selectedAssignment.correctText);
  ...
  const progress = selectedAssignment && selectedAssignment.correctText.length > 0 
    ? Math.min((typedText.length / selectedAssignment.correctText.length) * 100, 100) 
    : 0;
