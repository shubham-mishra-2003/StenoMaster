...
export const AssignmentsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getAssignments = useCallback(async (id?: string) => {
    ...
    try {
      const response = await fetch(id ? `/api/student/assignments?id=${id}` : "/api/student/assignments", {
        method: "GET",
      ...
      if (response.ok) {
        if (id) {
          setState((prev) => ({ ...prev, assignments: [result], loading: false }));
  ...
  const createAssignment = useCallback(async (assignment: Assignment) => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        setState((prev) => ({ ...prev, assignments: [...prev.assignments, { ...assignment, id: result.id }], loading: false }));
  ...
  const getScores = useCallback(async (studentId?: string) => {
    setState((prev) => ({ ...prev, loading: true, error: null }));
    try {
      const response = await fetch(studentId ? `/api/student/scores?studentId=${studentId}` : "/api/student/scores", {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      const result = await response.json();
      if (response.ok) {
        ...
        setState((prev) => ({ ...prev, scores: processedScores, loading: false }));
  ...
  const createScore = useCallback(async (score: Score) => {
    setState((prev) => ({ ...prev, loading: true, error: null }));
    try {
      const completedAt = score.completedAt instanceof Date ? score.completedAt : new Date(score.completedAt);
      ...
      const response = await fetch("/api/student/scores", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...score, completedAt: completedAt.toISOString() }),
      });
      const result = await response.json();
      if (response.ok) {
        setState((prev) => ({ ...prev, scores: [...prev.scores, { ...score, id: result.id }], loading: false }));
  ...
  useEffect(() => {
    const initializeData = async () => {
      if (!isAuthenticated || !user?.userId) {
        setState((prev) => ({ ...prev, assignments: [], scores: [], loading: false }));
        return;
      }
      try {
  ...
  return (
    <AssignmentContext.Provider
      value={{ ...state, getAssignments, createAssignment, updateAssignment, deleteAssignment, getScores, createScore }}
    >
      {children}
    </AssignmentContext.Provider>
  );
